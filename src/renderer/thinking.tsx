...
import { domToReact, type DOMNode, type Element as DomElement, type Text as DomText } from "html-react-parser";
...
export const thinkingRenderer: RendererMap = {
    ...
    ["think" as keyof HTMLElementTagNameMap]: (node: DomElement) => {
    ...
    [Node.TEXT_NODE]: (node: DomText) => {
        ...
        while ((match = thinkPattern.exec(text)) !== null) {
            ...
            parts.push(<ThinkingBlock key={`think-${key++}`}>{thinkingContent}</ThinkingBlock>);
