...
`m.replace` must satisfy "latest wins" semantics from the consumer perspective. Previously, async completion order could violate this. The update token enforces ordering by *logical update recency*, not promise resolution timing.
